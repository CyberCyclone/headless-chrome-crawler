...
class Crawler {
  ...
  async crawl() {
    ...
    let context = this;
    const screenshot = await Promise.race([this._screenshot(), new Promise(function (resolve, reject) {
         setTimeout(() => reject(new Error('Screenshot timeout')), context._options.timeout)
    })]);
