...
class HCCrawler extends EventEmitter {
  ...
  async _startRequest(options, depth, previousUrl) {
    ...
    if (skip) {
      this.emit(HCCrawler.Events.RequestSkipped, options, skip);
  ...
  async _skipRequest(options) {
    ...
    if (!allowedDomain) return "domain check failed";
    ...
    if (!shouldRequest) return "pre-request failed";
    return false;
  }
    ...
    if (!sitemapXml) {
      try {
        let rp_options = {
          ...
          headers: {
            'User-Agent': options.userAgent || "Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:68.0) Gecko/20100101 Firefox/68.0"
          }
        };
        sitemapXml = await rp(rp_options);
      } catch (error) {
        extend(error, { options, depth, previousUrl });
